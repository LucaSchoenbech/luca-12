...
export function Logo({ className = "", iconOnly = false, textOnly = false, coloredOnHover = false }: { className?: string, iconOnly?: boolean, textOnly?: boolean, coloredOnHover?: boolean }) {
  ...
  const LogoIcon = () => (
    <svg viewBox="0 0 100 100" className={`${iconOnly ? 'w-8 h-8' : 'w-20 h-20 md:w-24'} ${className}`} fill="none">
      ...
      <g filter={!isLight ? "url(#shadow)" : undefined}>
        <path d="M 48 90 C 35 85 15 70 15 50 C 20 46 28 52 30 58 C 34 68 40 72 48 74 Z" 
          className={isLight ? (coloredOnHover ? "fill-current opacity-80 group-hover:opacity-100 transition-all duration-500" : "fill-current opacity-80") : "fill-[url(#navy)]"} />
        <path d="M 52 90 C 65 85 85 70 85 50 C 80 46 72 52 70 58 C 66 68 60 72 52 74 Z" 
          className={isLight ? (coloredOnHover ? "fill-current opacity-80 group-hover:opacity-100 transition-all duration-500" : "fill-current opacity-80") : "fill-[url(#navy)]"} />
      </g>

      {/* Gold Top Half */}
      <g filter={!isLight ? "url(#shadow)" : undefined}>
        <path d="M 48 10 C 35 15 15 30 15 50 C 20 54 28 48 30 42 C 34 32 40 28 48 26 Z" 
          className={isLight ? (coloredOnHover ? "fill-current group-hover:fill-[url(#gold)] transition-all duration-500" : "fill-current") : "fill-[url(#gold)]"} />
        <path d="M 52 10 C 65 15 85 30 85 50 C 80 54 72 48 70 42 C 66 32 60 28 52 26 Z" 
          className={isLight ? (coloredOnHover ? "fill-current group-hover:fill-[url(#gold)] transition-all duration-500" : "fill-current") : "fill-[url(#gold)]"} />
      </g>
